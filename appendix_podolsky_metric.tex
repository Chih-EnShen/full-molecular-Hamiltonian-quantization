...
\end{equation}
every ingredient of which reduces, via Jacobi's formula (4.36) and the linearity (4.37), to the constant $\boldsymbol\zeta$'s and the linear $\boldsymbol\sigma(Q)$; the Watson sum rules then collapse (B.19) to the trace $\hat U=-\tfrac{\hbar^2}{8}\sum_\alpha\mu_{\alpha\alpha}$ of (4.38)~\cite{watson1968}.
...
\subsubsection*{B.6\quad Worked example: a bent triatomic}
...
\qquad a_\alpha^{b}\equiv\frac{\partial I_\alpha}{\partial Q_b}\Big|_{\mathrm{eq}}, \tag{B.21}
\end{equation}
...
\qquad \zeta^{b}_{ab}=-\zeta^{b}_{ba}, \tag{B.22}
\end{equation}
...
s(Q)\equiv(\zeta^{b}_{13}Q_3)^2+(\zeta^{b}_{23}Q_3)^2+(\zeta^{b}_{13}Q_1+\zeta^{b}_{23}Q_2)^2 . \tag{B.23}
\end{equation}
Note $s(Q)=\mathcal O(Q^2)$, so it does not affect the linear-order derivatives (B.21) but does enter the pseudopotential and the exact Jacobian.

\paragraph{Determinant, volume element, and its explicit derivatives.}
With $\mathbf I'$ diagonal in (B.23),
\begin{equation}
\gamma(Q)=\det\mathbf I'=I_a(Q)\,\bigl(I_b(Q)-s(Q)\bigr)\,I_c(Q),\qquad
\sqrt{g}=\sin\theta\,\sqrt{I_a\,(I_b-s)\,I_c}, \tag{B.24}
\end{equation}
...
\partial_{Q_1}s=2\zeta^{b}_{13}\bigl(\zeta^{b}_{13}Q_1+\zeta^{b}_{23}Q_2\bigr),\ \text{etc.}, \tag{B.25}
\end{equation}
...
\begin{equation}
\boxed{\;\hat U(Q)=-\frac{\hbar^2}{8}\Bigl[\frac{1}{I_a(Q)}+\frac{1}{I_b(Q)-s(Q)}+\frac{1}{I_c(Q)}\Bigr].\;}\tag{B.26}
\end{equation}
Expanding to the order at which it first varies with the coordinates, and using (B.20),
\begin{equation}
\hat U(Q)=-\frac{\hbar^2}{8}\Bigl(\frac{1}{I_a^{e}}+\frac{1}{I_b^{e}}+\frac{1}{I_c^{e}}\Bigr)
+\frac{\hbar^2}{8}\sum_{b=1,2}\Bigl(\frac{a_a^{b}}{(I_a^{e})^2}+\frac{a_b^{b}}{(I_b^{e})^2}+\frac{a_c^{b}}{(I_c^{e})^2}\Bigr)Q_b+\mathcal O(Q^2). \tag{B.27}
\end{equation}
The constant is an inconsequential shift of the electronic--vibrational zero; the linear term is a genuine $\hbar^2$ force on the normal coordinates --- the physical content of the Watson pseudopotential for this molecule --- produced entirely by differentiating $\sqrt{\gamma(Q)}$ through (B.25). Every step from the metric (4.15) to the surface (B.27) used only $\partial_\phi,\partial_\theta,\partial_\chi$ and $\partial_{Q_b}$.

\subsubsection*{B.7\quad Summary}
