...
\begin{enumerate}
  ...
  \item \textbf{Body-Fixed} — co-rotating frame + Routhian $\Rightarrow$ classical molecular $H$.
...
\begin{enumerate}
  ...
  \item parametrizing the nuclear shape by $3N_{\mathrm{nucl}}-6$ internal coordinates (exact).
\end{enumerate}
\vspace{0.5ex}
\centering\itshape The endpoint is the exact quantum $\hat H_{\mathrm{int}}$ — the rigorous starting point \\ for \emph{any} later approximation.
\end{frame}
...
\begin{equation*}
H = \frac{\mathbf{P}_{\mathrm{cm}}^{\,2}}{2M_N}\underbrace{-\frac{\mathbf{P}_{\mathrm{cm}}\!\cdot\!\sum_i\mathbf{p}_i}{M_N}}_{\text{artifact}}+\underbrace{\frac{(\sum_i\mathbf{p}_i)^2}{2M_N}}_{\text{mass polarization}}+\sum_j\frac{\mathbf{P}_j^{'2}}{2M_j}+\sum_i\frac{\mathbf{p}_i^{\,2}}{2m_e}+V. \tag{2.10}
\end{equation*}
\pause
Setting $\mathbf P_{\mathrm{cm}}=0$ (rest frame) gives the internal Hamiltonian
\begin{equation*}
\boxed{H_{\mathrm{int}} = \sum_j\frac{\mathbf{P}_j^{'2}}{2M_j}+\sum_i\frac{\mathbf{p}_i^{\,2}}{2m_e}+\underbrace{\frac{1}{2M_N}\Bigl(\sum_i\mathbf{p}_i\Bigr)^{2}}_{\text{electron mass polarization}}+V.}\tag{2.11}
\end{equation*}
\begin{block}{Why the mass-polarization term?}
It is the \emph{price} of referring electrons to the \emph{nuclear} COM (not the total COM): the electron momenta no longer commute additively with $M_N$. We keep it exactly.
\end{block}
\end{frame}

\begin{frame}{Lifting $\mathbf P_{\mathrm{cm}}=0$: a canonical transformation}
\setstage{1}
For $\mathbf P_{\mathrm{cm}}\neq0$, (2.10) carries two artifacts:
\begin{equation*}
H=\underbrace{\tfrac{\mathbf{P}_{\mathrm{cm}}^{\,2}}{2M_N}}_{(A)\text{ wrong mass}}\underbrace{-\tfrac{\mathbf{P}_{\mathrm{cm}}\cdot\sum_i\mathbf{p}_i'}{M_N}}_{(B)\text{ cross-coupling}}+\tfrac{(\sum_i\mathbf{p}_i')^{2}}{2M_N}+\sum_j\tfrac{\mathbf{P}_j^{'2}}{2M_j}+\sum_i\tfrac{\mathbf{p}_i^{'2}}{2m_e}+V. \tag{2.12}
\end{equation*}
\pause
A single canonical transformation generated by $G=\tfrac{m_e}{M_{\mathrm{tot}}}\mathbf{P}_{\mathrm{cm}}\!\cdot\!\sum_i\mathbf{r}_i'$ removes both:
\begin{equation*}
...
It relabels the COM from \emph{nuclear} to \emph{total} (Galilean boost), correcting $M_N\!\to\!M_{\mathrm{tot}}$ and killing (B) --- \emph{exactly, for any} $\mathbf P_{\mathrm{cm}}$. The internal physics is then frame-independent.
\end{block}
\end{frame}

\begin{frame}{The frame-independent internal Hamiltonian}
\setstage{1}
The result is structurally identical to the rest-frame form, but exact for \emph{any} $\mathbf P_{\mathrm{cm}}$:
\begin{equation*}
...
\end{equation*}
\begin{itemize}
  \item COM term now carries the \emph{correct} total mass $M_{\mathrm{tot}}=M_N+N_{\mathrm{elec}}m_e$.
  \item Internal part = nuclear KE + electron KE + mass polarization + Coulomb.
  \item $\mathbf P_{\mathrm{cm}}$ is a decoupled, conserved spectator.
\end{itemize}
\vspace{0.5ex}
\centering\itshape (2.22) is what we carry into the body-fixed frame.
\end{frame}
...
\begin{frame}{A Routhian: keep electrons in momentum form}
\setstage{2}
We need an action principle for the BF \emph{nuclear} coordinates --- but the electronic kinetic energy + mass polarization
$H_e=\sum_i\tilde{\mathbf p}_i'^{\,2}/2m_e+\tfrac{1}{2M_N}(\sum_i\tilde{\mathbf p}'_i)^2$
is a \emph{rotational scalar}. So Legendre-transform only the COM and nuclear momenta, leaving the electrons canonical $\Rightarrow$ a \alert{Routhian}:
\begin{equation*}
\boxed{\;\mathcal{R}=\tfrac12 M_{\mathrm{tot}}|\dot{\tilde{\mathbf{R}}}_{\mathrm{cm}}|^2+\tfrac12\sum_j M_j|\dot{\mathbf{R}}_j'|^2-H_e(\tilde{\mathbf{p}}'_i)-V.\;}\tag{3.6}
\end{equation*}
\begin{block}{Why a Routhian?}
It avoids converting the electron kinetic energy to velocity form and back. The electrons ride along as the scalar $H_e$ --- a Lagrangian in $(\tilde{\mathbf R}_{\mathrm{cm}},\mathbf R'_j)$, \emph{minus} a Hamiltonian in $(\mathbf r'_i,\tilde{\mathbf p}'_i)$.
\end{block}
\end{frame}
...
\begin{equation*}
\mathcal{R}=\tfrac12 M_{\mathrm{tot}}|\dot{\tilde{\mathbf{R}}}_{\mathrm{cm}}|^2
...
\begin{equation*}
\boxed{\;\mathcal{R}=\tfrac12 M_{\mathrm{tot}}|\dot{\tilde{\mathbf{R}}}_{\mathrm{cm}}|^2
...
\setstage{4}
\begin{enumerate}
  \item[\textbf{1.}] \alert{Classical}: lab $H$ (1.1) $\to$ frame-independent internal $\tilde H$ (2.22), COM split off exactly for any $\mathbf P_{\mathrm{cm}}$.
  ...
  \item[\textbf{4.}] \alert{Quantum $\hat H$}: the exact non-relativistic $\hat H_{\mathrm{int}}$ (4.14).
\end{enumerate}
\end{frame}

\begin{frame}{What we did --- and did not --- assume}
\setstage{4}
$\hat H_{\mathrm{int}}$ (4.14) is \alert{exact}. The only inputs were:
\begin{itemize}
  \item non-relativistic dynamics;
  \item the Eckart frame (a convention);
  \item $3N_{\mathrm{nucl}}-6$ internal coordinates (an exact parametrization).
\end{itemize}
\vspace{0.5ex}
