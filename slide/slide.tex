...
\author{Author: Claude Code \\ Presenter: Chih-En Shen}
% \author{Chih-En Shen}
\date{\today}
...
The Euler angles and $Q$ are \emph{curvilinear}. With the curvilinear volume element $\sqrt{|g|}\,d^nq$, the naive quantization $\hat\pi_A=-i\hbar\partial_A$ makes $\tfrac12 g^{AB}\hat\pi_A\hat\pi_B$ \textbf{non-Hermitian} ($g^{AB}$ depends on $q$ and does not commute with $\partial_A$).
\end{alertblock}
\end{frame}
...
\begin{frame}{Podolsky quantization: the recipe}
\setstage{3}
Three steps from the classical $T=\tfrac12 g^{AB}\pi_A\pi_B$ to the exact quantum $\hat T$:
\begin{enumerate}
  \item \textbf{Write the unique Hermitian operator} consistent with the curvilinear volume element $\sqrt{|g|}\,d^nq$ --- the Podolsky / Laplace--Beltrami form.
  \item \textbf{Identify $\sqrt{|g|}$} for the molecule from the block structure of $g_{AB}$.
  \item \textbf{Flatten the vibrational measure} via $\psi\to\psi_W$ --- the $Q$-Jacobian generates the Watson pseudopotential.
\end{enumerate}
\vspace{0.5ex}
\footnotesize The anomalous BF angular-momentum algebra ensures the rotational sector contributes \emph{no} extra potential.
\end{frame}

\begin{frame}{The Podolsky / Laplace--Beltrami prescription}
\setstage{3}
\textbf{Step~1.}\enspace The unique kinetic operator self-adjoint with respect to the volume element $\sqrt{|g|}\,d^nq$ is the Laplace--Beltrami operator:
\begin{equation*}
...
\setstage{3}
\textbf{Step~2.}\enspace Building $g_{AB}$ from the velocity-form kinetic energy and reducing the determinant by block-matrix identities gives
\begin{equation*}
\boxed{\;\sqrt{|g|}\,d^nq=\underbrace{\sin\theta\,d\phi\,d\theta\,d\chi}_{\text{Euler-angle measure}}\cdot\underbrace{\sqrt{\det\mathbf I'(Q)}\,\textstyle\prod_b dQ_b}_{\text{rot.--vib.\ Jacobian}}\cdot\underbrace{\textstyle\prod_i d^3\bar{\mathbf r}_i}_{\text{flat}}.\;}\tag{4.4}
\end{equation*}
...
\setstage{3}
\textbf{Step~3.}\enspace Rescale the wavefunction by the $Q$-Jacobian $\gamma\equiv\det\mathbf I'$ to work on the flat measure $d\mu_W=\sin\theta\,d\phi d\theta d\chi\prod_b dQ_b\prod_i d^3\bar{\mathbf r}_i$:
\begin{equation*}
...
$\psi_W$ is normalised with the \emph{flat} vibrational measure, so $\hat P_b=-i\hbar\partial_{Q_b}$ is Hermitian. The similarity does not commute through $\gamma(Q)$ --- the leftover is exactly the pseudopotential $\hat U(Q)$.
\end{block}
\end{frame}
...
\end{equation*}
(block-matrix determinant identity). Hence $g=1/\det\boldsymbol\mu=\det\mathbf I'\equiv\gamma(Q)$. Since the $\hat J_\alpha$ already carry $\sin\theta$, only $\gamma(Q)$ is rescaled: set $\ln g\to\ln\gamma$ in (4.10d).
\end{frame}
...
\end{aligned}}\tag{4.14}
\end{equation*}
\centering\footnotesize $d\mu_W=\sin\theta\,d\phi d\theta d\chi\prod_b dQ_b\prod_i d^3\bar{\mathbf r}_i$.
\end{frame}
