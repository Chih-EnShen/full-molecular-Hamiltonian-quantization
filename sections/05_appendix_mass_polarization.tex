...
With $\mathcal L = T - V$ and $V$ velocity-independent, differentiate the boxed $T$.

\noindent\emph{CoM:} only the first term carries $\dot{\mathbf R}_{\mathrm{cm}}$, so
\begin{equation}
\boxed{\;\mathbf P_{\mathrm{cm}} = \frac{\partial \mathcal L}{\partial\dot{\mathbf R}_{\mathrm{cm}}} = M_{\mathrm{tot}}\dot{\mathbf R}_{\mathrm{cm}} .\;} \tag{A.13}
\end{equation}
...
\begin{equation}
\boxed{\;\mathbf p'_i = \frac{\partial \mathcal L}{\partial\dot{\mathbf r}'_i}
...
\noindent\emph{Why the sign flips.} The electron block of the velocity-form kinetic energy is the quadratic form $\frac12\dot{\mathbf r}^{\prime\mathsf T}\mathsf A\,\dot{\mathbf r}'$ with the non-diagonal mass matrix
\begin{equation}
\mathsf A = m_e\Bigl(\mathbb{1} - \frac{m_e}{M_{\mathrm{tot}}}\mathsf J\Bigr),
\qquad \mathsf J_{il} = 1 \ \text{(all-ones block)}, \tag{A.25}
\end{equation}
and the Legendre transform replaces $\mathsf A$ by its \emph{inverse}:
\begin{equation}
\mathsf A^{-1} = \frac{1}{m_e}\Bigl(\mathbb{1} + \frac{m_e}{M_N}\mathsf J\Bigr)
\qquad\Longrightarrow\qquad
\frac12\,\mathbf p^{\prime\mathsf T}\mathsf A^{-1}\mathbf p'
= \sum_i\frac{\mathbf p_i^{'2}}{2m_e} + \frac{1}{2M_N}\Bigl(\sum_i\mathbf p'_i\Bigr)^2 , \tag{A.26}
\end{equation}
(one verifies $\mathsf A\mathsf A^{-1}=\mathbb{1}$ using $\mathsf J^2 = N_{\mathrm{elec}}\mathsf J$ and $M_{\mathrm{tot}} - N_{\mathrm{elec}}m_e = M_N$). The inversion turns the negative $-m_e^2/2M_{\mathrm{tot}}$ collective coupling into the positive $+1/2M_N$ mass polarization: same physics, opposite side of the Legendre transform.
