...
\begin{equation}
\mathbf{R}_j'=\mathsf{S}(\phi,\theta,\chi)\,\bar{\mathbf{R}}_j(Q),\qquad \mathbf{r}_i'=\mathsf{S}(\phi,\theta,\chi)\,\bar{\mathbf{r}}_i. \tag{3.1}
\end{equation}
The Eckart conditions $\sum_j M_j\bar{\mathbf{R}}_j=0$ and $\sum_j M_j\bar{\mathbf{R}}_j^{\mathrm{eq}}\!\times\bar{\mathbf{R}}_j=0$ fix the orientation of $\mathsf{S}$ from the \emph{nuclear} positions; the same $\mathsf{S}$ is then used to define BF electron coordinates $\bar{\mathbf{r}}_i$. The nuclear shape is parametrized by rectilinear mass-weighted \textbf{normal coordinates} $Q_b$,
\begin{equation}
...
BF angular velocity:
\begin{equation}
(\mathsf{S}^T\dot{\mathsf{S}})_{\alpha\beta}=-\epsilon_{\alpha\beta\gamma}\,\omega_\gamma \Longrightarrow \mathsf{S}^T\dot{\mathsf{S}}\,\mathbf{v}=\boldsymbol{\omega}\times\mathbf{v}. \tag{3.2}
\end{equation}
Velocities (using $|\mathsf{S}\mathbf{v}|^2=|\mathbf{v}|^2$):
\begin{equation}
\dot{\mathbf{R}}_j'=\mathsf{S}\bigl(\boldsymbol{\omega}\!\times\!\bar{\mathbf{R}}_j+\dot{\bar{\mathbf{R}}}_j\bigr),\qquad \dot{\mathbf{r}}_i'=\mathsf{S}\bigl(\boldsymbol{\omega}\!\times\!\bar{\mathbf{r}}_i+\dot{\bar{\mathbf{r}}}_i\bigr). \tag{3.3}
\end{equation}
...
Substitute the body-fixed velocities (3.3) into the nuclear term of (3.6); the COM term is untouched by the rotation. The electron Hamiltonian $H_e$ is a \emph{rotational scalar} --- it is built from the dot products $\mathbf{p}'_i\!\cdot\!\mathbf{p}'_l$ --- so under the passive rotation to the body frame it is unchanged in form. Defining the body-fixed electron momenta $\bar{\mathbf{p}}_i\equiv\mathsf{S}^{T}\mathbf{p}'_i$ (a proper rotation, so $|\bar{\mathbf{p}}_i|^2=|\mathbf{p}'_i|^2$ and $\sum_i\bar{\mathbf{p}}_i=\mathsf{S}^{T}\sum_i\mathbf{p}'_i$, whence $(\sum_i\bar{\mathbf{p}}_i)^2=(\sum_i\mathbf{p}'_i)^2$),
\begin{equation}
...
\end{equation}
in which only the nuclear kinetic term carries the body-fixed angular velocity $\boldsymbol{\omega}$. The potential is now manifestly $\mathsf{S}$-independent:
\begin{equation}
...
\;=\; \tfrac12\boldsymbol{\omega}^{T}\!\mathsf{I}_N\boldsymbol{\omega} \;+\; \boldsymbol{\omega}\!\cdot\!\mathbf{L}_N \;+\; T_{\mathrm{vib}}. \tag{3.9d}
\end{equation}
...
= \tfrac12\boldsymbol{\omega}^{T}\!\mathsf{I}_e\boldsymbol{\omega} + \boldsymbol{\omega}\!\cdot\!\mathbf{L}_e + T_{e}^{\mathrm{BF}},\qquad
-\tfrac{m_e^{2}}{2M_{\mathrm{tot}}}\bigl|\boldsymbol{\omega}\!\times\!\bar{\mathbf{r}}+\dot{\bar{\mathbf{r}}}\bigr|^{2}
= -\tfrac12\boldsymbol{\omega}^{T}\!\mathsf{I}_X\boldsymbol{\omega} - \boldsymbol{\omega}\!\cdot\!\mathbf{L}_X - T_{X}^{\mathrm{mp}}. \tag{3.9e}
\end{equation}

The constituent inertia tensors are
\begin{equation}
\mathsf{I}_N=\sum_j M_j(\bar R_j^2\mathbb{1}-\bar{\mathbf{R}}_j\bar{\mathbf{R}}_j^T),\qquad \mathsf{I}_e=m_e\!\sum_i(\bar r_i^2\mathbb{1}-\bar{\mathbf{r}}_i\bar{\mathbf{r}}_i^T), \tag{3.9}
\end{equation}
\begin{equation}
\mathsf{I}_X=\tfrac{m_e^2}{M_{\mathrm{tot}}}(\bar r^2\mathbb{1}-\bar{\mathbf{r}}\bar{\mathbf{r}}^T), \tag{3.10}
\end{equation}
the angular-momentum-like (velocity-form) quantities are
\begin{equation}
\mathbf{L}_N=\sum_jM_j\bar{\mathbf{R}}_j\!\times\!\dot{\bar{\mathbf{R}}}_j,\quad \mathbf{L}_e=m_e\!\sum_i\bar{\mathbf{r}}_i\!\times\!\dot{\bar{\mathbf{r}}}_i,\quad \mathbf{L}_X=\tfrac{m_e^2}{M_{\mathrm{tot}}}\bar{\mathbf{r}}\!\times\!\dot{\bar{\mathbf{r}}}, \tag{3.11}
\end{equation}
and the ``vibrational'' kinetic pieces are
\begin{equation}
T_{\mathrm{vib}}=\tfrac12\!\sum_jM_j|\dot{\bar{\mathbf{R}}}_j|^2=\tfrac12\!\sum_b\dot Q_b^2,\quad T_{e}^{\mathrm{BF}}=\tfrac{m_e}{2}\!\sum_i|\dot{\bar{\mathbf{r}}}_i|^2,\quad T_X^{\mathrm{mp}}=\tfrac{m_e^2}{2M_{\mathrm{tot}}}|\dot{\bar{\mathbf{r}}}|^2. \tag{3.12}
\end{equation}
...
\begin{equation}
\boxed{\;\mathcal{R}=\tfrac12 M_{\mathrm{tot}}|\dot{\mathbf{R}}_{\mathrm{cm}}|^2+\underbrace{\tfrac12\boldsymbol{\omega}^{T}\mathsf{I}_N\,\boldsymbol{\omega}+\boldsymbol{\omega}\!\cdot\!\mathbf{L}_N+T_{\mathrm{vib}}}_{\text{nuclear, velocity form}}\;-\;H_e\;-\;V.\;} \tag{3.13}
\end{equation}
The rotational kernel is the \textbf{nuclear} inertia $\mathsf{I}_N$ \emph{alone}: because the electrons are carried as the scalar $H_e$ rather than expanded in $\boldsymbol{\omega}$, no electronic or mass-polarization inertia ($\mathsf{I}_e$, $\mathsf{I}_X$) enters the kernel. The electrons couple to rotation only \emph{kinematically}, through their angular momentum, which we identify next. The body-fixed electron momenta and the \textbf{electronic angular momentum} are
\begin{equation}
\bar{\mathbf{p}}_i=\mathsf{S}^{T}\mathbf{p}'_i,\qquad \mathbf{L}_{\mathrm{elec}}\equiv\sum_i\bar{\mathbf{r}}_i\!\times\!\bar{\mathbf{p}}_i. \tag{3.14}
\end{equation}
\textbf{Remark.} The electronic and mass-polarization inertia/angular-momentum quantities $\mathsf{I}_e,\mathsf{I}_X,\mathbf{L}_e,\mathbf{L}_X$ of (3.9)--(3.11), and the velocity-form decomposition (3.9e), are needed \emph{only} in the cross-check of \S\,3.6; they play no role in the Routhian (3.13) itself.
...
\end{equation}
The right side is a purely nuclear-velocity quantity, with the \emph{nuclear} inertia $\mathsf{I}_N$ as rotational kernel. The entire electronic and mass-polarization content sits on the left, in the kinematic shift $\mathbf{J}\to\mathbf{J}-\mathbf{L}_{\mathrm{elec}}$; it never enters the kernel, because in the Routhian (3.13) the electrons were carried as the scalar $H_e$ and contributed no inertia at all.

\subsubsection{Cross-check: the $\mathsf{I}_e/\mathsf{I}_X$ cancellation of the velocity-form route}
...
\begin{equation}
\mathbf{L}_{\mathrm{elec}}=(\mathsf{I}_e-\mathsf{I}_X)\boldsymbol{\omega}+(\mathbf{L}_e-\mathbf{L}_X), \tag{3.18}
\end{equation}
with $\mathsf{I}_e,\mathsf{I}_X,\mathbf{L}_e,\mathbf{L}_X$ from (3.9)--(3.11). The velocity-form total angular momentum would read $\mathbf{J}=(\mathsf{I}_N+\mathsf{I}_e-\mathsf{I}_X)\boldsymbol{\omega}+(\mathbf{L}_N+\mathbf{L}_e-\mathbf{L}_X)$ [from (3.9d)+(3.9e)], so
\begin{equation*}
\mathbf{J}-\mathbf{L}_{\mathrm{elec}}=\mathsf{I}_N\boldsymbol{\omega}+\mathbf{L}_N+\underbrace{\bigl[(\mathsf{I}_e-\mathsf{I}_X)\boldsymbol{\omega}+(\mathbf{L}_e-\mathbf{L}_X)\bigr]-\mathbf{L}_{\mathrm{elec}}}_{=\,0\ \text{by (3.18)}}=\mathsf{I}_N\boldsymbol{\omega}+\mathbf{L}_N,
\end{equation*}
reproducing (3.19): the electronic inertia $\mathsf{I}_e$ and the mass-polarization inertia $\mathsf{I}_X$ cancel \emph{exactly}. This ``algebraic miracle'' of the Eckart construction is automatic in the mixed treatment, where $\mathsf{I}_e$ and $\mathsf{I}_X$ never arise.

\subsubsection{Inversion: the Watson reciprocal inertia}

To invert (3.19) for $\boldsymbol{\omega}$ we must re-express the velocity-form $\mathbf{L}_N$ in canonical variables. In normal coordinates $\mathbf{L}_N=\sum_jM_j\bar{\mathbf{R}}_j\!\times\!\dot{\bar{\mathbf{R}}}_j=\sum_b\boldsymbol{\sigma}_b\,\dot Q_b$; eliminating $\dot Q_b=P_b-\boldsymbol{\omega}\!\cdot\!\boldsymbol{\sigma}_b$ from (3.17b),
\begin{equation}
\mathbf{L}_N=\sum_b\boldsymbol{\sigma}_b\bigl(P_b-\boldsymbol{\omega}\!\cdot\!\boldsymbol{\sigma}_b\bigr)=\boldsymbol{\pi}-\Msig\Msig^{\mathsf T}\boldsymbol{\omega},\qquad
\boldsymbol{\pi}\equiv\sum_b\boldsymbol{\sigma}_bP_b,\quad \Msig\Msig^{\mathsf T}\equiv\sum_b\boldsymbol{\sigma}_b\boldsymbol{\sigma}_b^{\mathsf T}, \tag{3.20a}
\end{equation}
with $\boldsymbol{\pi}$ the \textbf{field-free vibrational angular momentum}. Substituting into (3.19), $\mathbf{J}-\mathbf{L}_{\mathrm{elec}}=(\mathsf{I}_N-\Msig\Msig^{\mathsf T})\boldsymbol{\omega}+\boldsymbol{\pi}\equiv\mathsf{I}'\boldsymbol{\omega}+\boldsymbol{\pi}$, so
\begin{equation}
\boldsymbol{\omega}=\Mmu\bigl(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}\bigr),\qquad
\boxed{\;\Mmu\equiv(\mathsf{I}')^{-1}=\bigl(\mathsf{I}_N-\Msig\Msig^{\mathsf T}\bigr)^{-1},\;} \tag{3.20}
\end{equation}
the \textbf{Watson reciprocal-inertia tensor}~\cite{watson1968}. The rotation--vibration coupling is now carried by the modified inertia $\mathsf{I}'$ and the uniform shift $\mathbf{J}\to\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}$ --- with no Wilson $G$-matrix.
...
\begin{equation*}
H_{\mathrm{int}}=T_N^{\mathrm{rot+vib}}+H_e+V,\qquad T_N^{\mathrm{rot+vib}}\equiv\tfrac12\boldsymbol{\omega}^{T}\mathsf{I}_N\boldsymbol{\omega}+\boldsymbol{\omega}\!\cdot\!\mathbf{L}_N+T_{\mathrm{vib}},
\end{equation*}
...
\end{equation*}
Inserting $\boldsymbol{\omega}=\Mmu(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi})$ from (3.20),
\begin{equation}
\boxed{\;T_N^{\mathrm{rot+vib}}\Big|_{\mathrm{canonical}} \;=\; \tfrac12(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi})^{T}\Mmu\,(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}) + \tfrac12\sum_{b}P_b^2.\;} \tag{3.20d}
\end{equation}
The vibrational kinetic energy is the \emph{bare} $\tfrac12\sum_bP_b^2$; the entire rotation--vibration coupling is carried by $\boldsymbol{\pi}$ and by the Watson inertia $\Mmu=(\mathsf{I}_N-\Msig\Msig^{\mathsf T})^{-1}$ --- no Wilson $G$-matrix survives.
...
\end{equation}
Because $H_e$ is built from the rotation-invariant dot products $\bar{\mathbf{p}}_i\!\cdot\!\bar{\mathbf{p}}_l$ and $\bar{\mathbf{p}}_i=\mathsf{S}^{T}\mathbf{p}'_i$ is a passive rotation, its body-fixed form is \emph{identical} to the internal Hamiltonian's form (2.13) under $\mathbf{p}'_i\to\bar{\mathbf{p}}_i$ --- no Legendre transform or angular-momentum expansion is needed. The mass polarization survives the body-fixed transformation intact in functional form.
...
\boxed{
\begin{aligned}
H_{\mathrm{int}}\;=\;& \tfrac12\bigl(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}\bigr)^T\Mmu(Q)\bigl(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}\bigr) \\
& +\tfrac12\!\sum_{b}P_b^2 \\
...
\begin{itemize}
    \item $\Mmu=(\mathsf{I}_N-\Msig\Msig^{\mathsf T})^{-1}$ is the \textbf{Watson reciprocal-inertia tensor}, built from nuclear quantities alone: \emph{no} electron or mass-polarization corrections enter the rotational kernel --- they were absorbed into $\mathbf{L}_{\mathrm{elec}}$ via (3.19).
    \item The electron contribution to rotation appears entirely as the Coriolis-like coupling $-\mathbf{J}^T\Mmu\mathbf{L}_{\mathrm{elec}}$.
...
$$
\tfrac12\bigl(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}\bigr)^T\Mmu\bigl(\mathbf{J}-\mathbf{L}_{\mathrm{elec}}-\boldsymbol{\pi}\bigr)=
$$
$$
\underbrace{\tfrac12\mathbf{J}^T\Mmu\mathbf{J}}_{\text{rotation}}
\underbrace{-\mathbf{J}^T\Mmu\boldsymbol{\pi}}_{\text{rot–vib Coriolis}}
\underbrace{-\mathbf{J}^T\Mmu\mathbf{L}_{\mathrm{elec}}}_{\text{rot–elec Coriolis}}
\underbrace{+\boldsymbol{\pi}^{T}\Mmu\mathbf{L}_{\mathrm{elec}}}_{\text{vib–elec angular}}
+\tfrac12\boldsymbol{\pi}^{T}\Mmu\boldsymbol{\pi}+\tfrac12\mathbf{L}_{\mathrm{elec}}^{T}\Mmu\mathbf{L}_{\mathrm{elec}}.
$$

All couplings in (3.22), referred to the standard analyses~\cite{watson1968,bunkerjensen}:
\begin{enumerate}
    \item \textbf{Pure rotation, anisotropic top:} $\tfrac12\mathbf{J}^T\Mmu(Q)\mathbf{J}$ --- $Q$-dependent moments of inertia (rot–vib coupling through inertia).
    \item \textbf{Rot–vib Coriolis}~\cite{wilson1955}: $-\mathbf{J}^T\Mmu\boldsymbol{\pi}$.
    \item \textbf{Rot–electron Coriolis:} $-\mathbf{J}^T\Mmu\mathbf{L}_{\mathrm{elec}}$ --- the kinetic source of $\Lambda$-doubling~\cite{mullikenchristy1931,vanvleck1951}, Renner–Teller~\cite{renner1934}, and spin–rotation~\cite{curl1965} analogues.
    \item \textbf{Vib–electron angular coupling:} $\boldsymbol{\pi}^{T}\Mmu\mathbf{L}_{\mathrm{elec}}$ --- exact non-adiabatic angular coupling intrinsic to BF embedding~\cite{bornoppenheimer1927,bornhuang1954,littlejohn1997}.
    \item \textbf{Quadratic vibrational angular momentum:} $\tfrac12\boldsymbol{\pi}^{T}\Mmu\boldsymbol{\pi}$.
    \item \textbf{Quadratic electronic angular momentum:} $\tfrac12\mathbf{L}_{\mathrm{elec}}^{T}\Mmu\mathbf{L}_{\mathrm{elec}}$.
...
Compared to the previous (electrons-in-SF) framework: kinetic rot–electron coupling has \emph{replaced} potential rot–electron coupling. $V_{Ne}$ is now manifestly $\mathsf{S}$-independent --- the canonical chemistry advantage.\newpage
