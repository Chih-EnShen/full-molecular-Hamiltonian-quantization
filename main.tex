...
\section{Transformation to the Total Center-of-Mass}

\subsection{New coordinates}

Define the \emph{total} (nuclei $+$ electrons) COM, and refer every particle --- nuclear and electronic alike --- to it:
...
\end{equation}
The relative vectors are linearly dependent: multiplying each equation in (2.2) by its mass and summing, the definition (2.1) forces
\begin{equation}
\sum_j M_j\mathbf{R}'_j + m_e\sum_i\mathbf{r}'_i = 0. \tag{2.3}
\end{equation}
Unlike the nuclear-COM construction, the constraint now ties \emph{all} particles together, electrons included.
...
\begin{equation}
F_2 = \mathbf{P}_{\mathrm{cm}}\cdot\mathbf{R}_{\mathrm{cm}}(\mathbf{R}_j,\mathbf{r}_i) + \sum_j\mathbf{P}'_j\cdot\mathbf{R}'_j(\mathbf{R}_j,\mathbf{r}_i) + \sum_i\mathbf{p}'_{\,i}\cdot\mathbf{r}'_i(\mathbf{R}_j,\mathbf{r}_i). \tag{2.4}
\end{equation}
Then $\mathbf{P}_j = \partial F_2/\partial\mathbf{R}_j$, $\mathbf{p}_i = \partial F_2/\partial\mathbf{r}_i$. Every new coordinate in (2.1)--(2.2) is linear in the old ones, so the Jacobian blocks are constant multiples of $\mathbb{1}$. Differentiating with respect to a nuclear position,
\begin{equation}
\frac{\partial\mathbf{R}_{\mathrm{cm}}}{\partial\mathbf{R}_j} = \frac{M_j}{M_{\mathrm{tot}}}\mathbb{1},\qquad \frac{\partial\mathbf{R}'_k}{\partial\mathbf{R}_j} = \Bigl(\delta_{jk}-\frac{M_j}{M_{\mathrm{tot}}}\Bigr)\mathbb{1},\qquad \frac{\partial\mathbf{r}'_l}{\partial\mathbf{R}_j} = -\frac{M_j}{M_{\mathrm{tot}}}\mathbb{1}, \tag{2.5a}
\end{equation}
and with respect to an electron position,
\begin{equation}
\frac{\partial\mathbf{R}_{\mathrm{cm}}}{\partial\mathbf{r}_i} = \frac{m_e}{M_{\mathrm{tot}}}\mathbb{1},\qquad \frac{\partial\mathbf{R}'_k}{\partial\mathbf{r}_i} = -\frac{m_e}{M_{\mathrm{tot}}}\mathbb{1},\qquad \frac{\partial\mathbf{r}'_l}{\partial\mathbf{r}_i} = \Bigl(\delta_{il}-\frac{m_e}{M_{\mathrm{tot}}}\Bigr)\mathbb{1}. \tag{2.5b}
\end{equation}
Note the symmetry between (2.5a) and (2.5b): because the COM contains \emph{every} particle, moving any one of them --- nucleus or electron --- drags $\mathbf{R}_{\mathrm{cm}}$ and hence shifts \emph{all} the relative coordinates. One obtains
\begin{equation}
\mathbf{P}_j = \frac{M_j}{M_{\mathrm{tot}}}\Bigl(\mathbf{P}_{\mathrm{cm}}-\textstyle\sum_k\mathbf{P}'_k-\sum_l\mathbf{p}'_l\Bigr)+\mathbf{P}'_j,\qquad
\mathbf{p}_i = \frac{m_e}{M_{\mathrm{tot}}}\Bigl(\mathbf{P}_{\mathrm{cm}}-\textstyle\sum_k\mathbf{P}'_k-\sum_l\mathbf{p}'_l\Bigr)+\mathbf{p}'_i. \tag{2.6}
\end{equation}
The constraint dual to (2.3) is $\sum_k\mathbf{P}'_k + \sum_l\mathbf{p}'_l = 0$, so
\begin{equation}
\boxed{\;\mathbf{P}_j = \frac{M_j}{M_{\mathrm{tot}}}\,\mathbf{P}_{\mathrm{cm}}+\mathbf{P}'_j,\qquad \mathbf{p}_i = \frac{m_e}{M_{\mathrm{tot}}}\,\mathbf{P}_{\mathrm{cm}}+\mathbf{p}'_i.\;} \tag{2.7}
\end{equation}

Total momentum: summing (2.7) over all particles, the mass fractions add to $(M_N + N_{\mathrm{elec}}m_e)/M_{\mathrm{tot}} = 1$ and the primed momenta cancel by the dual constraint, so $\sum_j\mathbf{P}_j+\sum_i\mathbf{p}_i = \mathbf{P}_{\mathrm{cm}}$: each particle carries its mass-fraction share of the total momentum plus its internal momentum.

\subsection{Hamiltonian after the transformation}

Insert (2.7) into $T_N$ and $T_e$ in (1.1). For the nuclei,
\begin{align}
T_N = \sum_j\frac{\mathbf{P}_j^{\,2}}{2M_j}
&= \sum_j\frac{1}{2M_j}\left[\frac{M_j}{M_{\mathrm{tot}}}\mathbf{P}_{\mathrm{cm}}+\mathbf{P}'_j\right]^{2} \nonumber\\
&= \frac{M_N}{M_{\mathrm{tot}}}\,\frac{\mathbf{P}_{\mathrm{cm}}^{\,2}}{2M_{\mathrm{tot}}} \;+\; \frac{\mathbf{P}_{\mathrm{cm}}}{M_{\mathrm{tot}}}\!\cdot\!\sum_j\mathbf{P}'_j \;+\; \sum_j\frac{\mathbf{P}_j^{'2}}{2M_j}, \tag{2.8}
\end{align}
using $\sum_j M_j = M_N$ in the first term. The electrons give the identical structure with $M_j\to m_e$, $\mathbf{P}'_j\to\mathbf{p}'_i$:
\begin{equation}
T_e = \sum_i\frac{\mathbf{p}_i^{\,2}}{2m_e} = \frac{N_{\mathrm{elec}}\,m_e}{M_{\mathrm{tot}}}\,\frac{\mathbf{P}_{\mathrm{cm}}^{\,2}}{2M_{\mathrm{tot}}} \;+\; \frac{\mathbf{P}_{\mathrm{cm}}}{M_{\mathrm{tot}}}\!\cdot\!\sum_i\mathbf{p}'_i \;+\; \sum_i\frac{\mathbf{p}_i^{'2}}{2m_e}. \tag{2.9}
\end{equation}
Adding (2.8) and (2.9): the two mass fractions sum to $(M_N+N_{\mathrm{elec}}m_e)/M_{\mathrm{tot}}=1$, and the two cross terms combine into
\begin{equation}
\frac{\mathbf{P}_{\mathrm{cm}}}{M_{\mathrm{tot}}}\cdot\underbrace{\Bigl(\sum_j\mathbf{P}'_j+\sum_i\mathbf{p}'_i\Bigr)}_{=\,0\ \text{(dual constraint)}} = 0, \tag{2.10}
\end{equation}
so no COM--internal cross-coupling survives. Combining with $V$:
\begin{equation}
H = \frac{\mathbf{P}_{\mathrm{cm}}^{\,2}}{2M_{\mathrm{tot}}} + \sum_j\frac{\mathbf{P}_j^{'2}}{2M_j} + \sum_i\frac{\mathbf{p}_i^{'2}}{2m_e} + V(\mathbf{R}'_j,\mathbf{r}'_i). \tag{2.11}
\end{equation}
The potential is unchanged in form because $\mathbf{R}'_j-\mathbf{R}'_k = \mathbf{R}_j-\mathbf{R}_k$, $\mathbf{R}'_j-\mathbf{r}'_i = \mathbf{R}_j-\mathbf{r}_i$, $\mathbf{r}'_i-\mathbf{r}'_l = \mathbf{r}_i-\mathbf{r}_l$. Because $H$ is independent of $\mathbf{R}_{\mathrm{cm}}$, $\mathbf{P}_{\mathrm{cm}}$ is conserved, and the COM motion separates \emph{exactly, for any} $\mathbf{P}_{\mathrm{cm}}$: the free term carries the correct total mass $M_{\mathrm{tot}}$, there is no cross-coupling, and no rest-frame choice is ever needed. The internal Hamiltonian is
\begin{equation}
\boxed{\;H_{\mathrm{int}} = \sum_j\frac{\mathbf{P}_j^{'2}}{2M_j} + \sum_i\frac{\mathbf{p}_i^{'2}}{2m_e} + V(\mathbf{R}'_j,\mathbf{r}'_i),\;}\tag{2.12}
\end{equation}
subject to the constraint pair (2.3) and its dual $\sum_j\mathbf{P}'_j+\sum_i\mathbf{p}'_i=0$.

\subsection{Remark: where the mass polarization went}

Referred to the total COM, the internal kinetic energy in (2.12) is \emph{diagonal}: no mass-polarization term and no $\mathbf{P}_{\mathrm{cm}}$ cross-coupling ever appear, in contrast with the nuclear-COM route, where the electron momenta pick up the term $(2M_N)^{-1}(\sum_i\mathbf{p}'_i)^2$. The price is paid elsewhere. In the nuclear-COM construction the constraint $\sum_j M_j\mathbf{R}'_j=0$ involves the nuclei only, and the electron coordinates are independent; here the single relation (2.3) ties \emph{all} $N_{\mathrm{nucl}}+N_{\mathrm{elec}}$ relative vectors together, nuclei and electrons alike. Eliminating this redundancy --- solving (2.3) for one particle's coordinate, or re-referring the electrons to the nuclear COM --- reintroduces off-diagonal (mass-polarization-type) kinetic couplings. A diagonal kinetic energy and an unconstrained coordinate set cannot be had simultaneously: the total-COM route stores the complexity in the constraint, the nuclear-COM route stores it in the mass-polarization term. The choice matters downstream: a body-fixed frame anchored to the nuclei alone no longer commutes trivially with the constraint (2.3), so the Eckart construction of Section 3 must be revisited in these coordinates.
